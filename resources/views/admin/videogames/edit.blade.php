                ...
                <div class="col-6 border-start border-dark">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control border border-dark" id="price"
                            name="price" value="{{ old('price', $videogame->price) }}">
                    </div>
                    {{-- PLATFORM --}}
                    <div class="mb-3">
                        <label for="platform" class="form-label">Platform</label>
                        <input type="text" class="form-control border border-dark" id="platform" name="platform"
                            value="{{ old('platform', $videogame->platform) }}">
                    </div>
                    {{-- DESCRIPTION --}}
                    <div class="mb-3">
                        <label for="description" class="form-label">Description</label>
                        <textarea class="form-control border border-dark" id="description" name="description" rows="2">{{ old('description', $videogame->description) }}"</textarea>
                    </div>
                    {{-- AGE RATING --}}
                    <div class="mb-3">
                        ...
                        <select class="form-select border border-dark" id="age_rating" name="age_rating">
                            <option value="3"
                                {{ old('age_rating', $videogame->age_rating) == '3' ? 'selected' : '' }}>3</option>
                            <option value="6"
                                {{ old('age_rating', $videogame->age_rating) == '6' ? 'selected' : '' }}>6</option>
                            <option value="9"
                                {{ old('age_rating', $videogame->age_rating) == '9' ? 'selected' : '' }}>9</option>
                            <option value="12"
                                {{ old('age_rating', $videogame->age_rating) == '12' ? 'selected' : '' }}>12</option>
                            <option value="16"
                                {{ old('age_rating', $videogame->age_rating) == '16' ? 'selected' : '' }}>16</option>
                            <option value="18"
                                {{ old('age_rating', $videogame->age_rating) == '18' ? 'selected' : '' }}>18</option>
                        </select>
                    </div>
                    {{-- VOTE --}}
                    <div class="mb-5">
                        ...
                        <input type="number" class="form-control border border-dark" id="vote" name="vote"
                            value="{{ old('vote', $videogame->vote) }}">
                    </div>
                    {{-- FORM ACTIONS --}}
                    <div class="d-flex justify-content-end pt-2">
                        <button type="reset" class="btn btn-secondary"><i
                                class="bi bi-arrow-clockwise"></i>Reset</button>
                        <button type="submit" class="btn btn-success ms-2"><i class="bi bi-plus-lg"></i>Update</button>
                    </div>
                </div>
            </div>
